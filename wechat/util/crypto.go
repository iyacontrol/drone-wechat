...
//EncryptMsg 加密消息
func EncryptMsg(random, rawJSONMsg []byte, corpID, aesKey string) (encrtptMsg []byte, err error) {
	defer func() {
		if e := recover(); e != nil {
	...
	ciphertext := AESEncryptMsg(random, rawJSONMsg, corpID, key)
	encrtptMsg = []byte(base64.StdEncoding.EncodeToString(ciphertext))
	return
}

//AESEncryptMsg ciphertext = AES_Encrypt[random(16B) + msg_len(4B) + rawJSONMsg + corpID]
//参考：github.com/chanxuehong/wechat.v2
func AESEncryptMsg(random, rawJSONMsg []byte, corpID string, aesKey []byte) (ciphertext []byte) {
	const (
		BlockSize = 32            // PKCS#7
		BlockMask = BlockSize - 1 // BLOCK_SIZE 为 2^n 时, 可以用 mask 获取针对 BLOCK_SIZE 的余数
	)

	corpIDOffset := 20 + len(rawJSONMsg)
	contentLen := corpIDOffset + len(corpID)
	...
	encodeNetworkByteOrder(plaintext[16:20], uint32(len(rawJSONMsg)))
	copy(plaintext[20:], rawJSONMsg)
	copy(plaintext[corpIDOffset:], corpID)
...
//DecryptMsg 消息解密
func DecryptMsg(corpID, encryptedMsg, aesKey string) (random, rawMsgXMLBytes []byte, err error) {
	defer func() {
		if e := recover(); e != nil {
			err = fmt.Errorf("panic error: err=%v", e)
			return
	...
	var encryptedMsgBytes, key, getcorpIDBytes []byte
	...
	random, rawMsgXMLBytes, getcorpIDBytes, err = AESDecryptMsg(encryptedMsgBytes, key)
	if err != nil {
		err = fmt.Errorf("消息解密失败,%v", err)
		return
	}
	if corpID != string(getcorpIDBytes) {
		err = fmt.Errorf("消息解密校验corpID失败")
		return
	}
	return
}
...
// AESDecryptMsg ciphertext = AES_Encrypt[random(16B) + msg_len(4B) + rawJSONMsg + corpID]
//参考：github.com/chanxuehong/wechat.v2
func AESDecryptMsg(ciphertext []byte, aesKey []byte) (random, rawJSONMsg, corpID []byte, err error) {
	const (
		BlockSize = 32            // PKCS#7
		BlockMask = BlockSize - 1 // BLOCK_SIZE 为 2^n 时, 可以用 mask 获取针对 BLOCK_SIZE 的余数
	)
	...
	// 反拼接
	// len(plaintext) == 16+4+len(rawJSONMsg)+len(corpID)
	if len(plaintext) <= 20 {
		err = fmt.Errorf("plaintext too short, the length is %d", len(plaintext))
		return
	}
	rawJSONMsgLen := int(decodeNetworkByteOrder(plaintext[16:20]))
	if rawJSONMsgLen < 0 {
		err = fmt.Errorf("incorrect msg length: %d", rawJSONMsgLen)
		return
	}
	corpIDOffset := 20 + rawJSONMsgLen
	if len(plaintext) <= corpIDOffset {
		err = fmt.Errorf("msg length too large: %d", rawJSONMsgLen)
		return
	}

	random = plaintext[:16:20]
	rawJSONMsg = plaintext[20:corpIDOffset:corpIDOffset]
	corpID = plaintext[corpIDOffset:]
	return
}
